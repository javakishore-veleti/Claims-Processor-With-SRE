...
> Detailed architecture diagrams and design documentation for the Claims-Processor-With-SRE microservice.
> This content was extracted from CLAUDE.md to keep the operational reference lean.
...
## Detailed Architecture: Claims Intake Processing with AWS Bedrock Data Automation
...
flowchart TB
    subgraph StaffReview["Admin Portal -- Extraction Review"]
        ...
        SideBySide["Side-by-Side:<br/>Original Document vs Extracted Fields"]
        BoundingBox["Click Field to Highlight<br/>Source in Original Document<br/>(Visual Grounding)"]
...
flowchart TB
    subgraph Settlement["Settlement Processing"]
        ...
        CloseStage["Advance Stage<br/>SETTLEMENT then CLOSED"]
    end
    ...
    subgraph CustomerPortal["Customer Portal (Angular)"]
        ...
        Appeal["File Appeal<br/>(Stage then APPEAL)"]
    end

    subgraph FeedbackLoop["Knowledge Feedback Loop"]
        IngestToKB["Ingest Completed Claim<br/>then Bedrock Knowledge Base"]
    ...
    subgraph Notifications["Kafka then Notifications"]
...
- Security best practices including vulnerability scanning and secure coding
