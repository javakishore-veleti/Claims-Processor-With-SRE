 ...
 * Provides read-only privilege checking for the member portal.
 */
@Slf4j
@Service
...
public class EntitlementsApiClient {
    ...
    public ApiResponse<JsonNode> validateAccess(String userId, String resource) {
        log.info("Validating access for user {} on resource {}", userId, resource);
        return serviceClient.get(SERVICE_NAME, BASE_PATH + "/users/" + userId + "/access?resource=" + resource, JsonNode.class);
    }
}
